(() => {
  ...
  function importSelectedICATMatch() {
    try {
      const raw = sessionStorage.getItem('icat_selected_match');
      if (!raw) return;
      const match = JSON.parse(raw);
      if (!match || !match.forceImport) return;
      const teamA = String(match.teamA || 'Team A');
      const teamB = String(match.teamB || 'Team B');
      const playersA = Array.isArray(match.playersA) && match.playersA.length >= 2 ? match.playersA : defaultPlayers(teamA);
      const playersB = Array.isArray(match.playersB) && match.playersB.length >= 2 ? match.playersB : defaultPlayers(teamB);
      const oversLimit = Math.max(1, Math.min(100, Number(match.overs) || 20));
      const batFirst = match.batFirst === 'B' ? 'B' : 'A';
      ...
      state = {
        version: 10, configured: true, scorer: String(match.scorer || 'Scorer'),
        teamA, teamB, playersA, playersB, oversLimit, batFirst,
        currentInnings: 0, target: null, matchComplete: false, result: '',
      ...
      console.warn('ICAT match import skipped', error);
    }
  }

  importSelectedICATMatch();
  let eventState = { type: null, values: {} };
  let batterEditTarget = 'striker';

  const current = () => state.innings[state.currentInnings];

  const APP_ROLE_KEY = 'icat_role_v21';
  const APP_STATE_KEY = 'icat-demo-v21';
  const APP_LOGIN_EMAIL_KEY = 'icat_login_email_v32';
  ...
  function selectedMatchContext() {
    try { return JSON.parse(sessionStorage.getItem(SCORING_SESSION_KEY) || 'null'); }
    catch { return null; }
  }

  function isAuthorizedScorer() {
    const team = authorizedAdminTeam();
    const selected = selectedMatchContext();
    const inn = state.innings?.[state.currentInnings];
    if (!team || !selected || !inn) return false;
    if (![selected.teamA, selected.teamB].includes(team)) return false;
    return inn.battingTeam === team;
  }

  function requireAuthorizedScorer() {
    if (isAuthorizedScorer()) return true;
    toast('Only the batting team admin can score this match.');
    return false;
  }

  function syncParentScoringState() {
    try {
      const selected = selectedMatchContext();
      const inn = state.innings?.[state.currentInnings];
      if (!selected || !inn || !selected.id) return;
      selected.battingTeam = inn.battingTeam;
      selected.currentInnings = state.currentInnings;
  ...
  function canScore() {
    if (!requireAuthorizedScorer()) return false;
    if (!state.configured) {
      openSetup();
      return false;
    }
  ...
  function render() {
    ...
    const authorized = isAuthorizedScorer();
    const disabled = !authorized || !state.configured || state.matchComplete || inn.complete;
    $$('.run-key,#wicketBtn,#wideBtn,#extrasBtn,#endInningsBtn').forEach(btn => btn.disabled = disabled);
    $('undoBtn').disabled = !authorized || !state.history.length;
    ['swapStrikerBtn','swapNonStrikerBtn','setupBtn','menuEditStriker','menuEditNonStriker','menuEditBowler','menuChangeScorer','menuResetMatch'].forEach(id => { const el = $(id); if (el) el.disabled = !authorized; });
  }
